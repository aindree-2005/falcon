...
def compute_swd_distance(z1, z2, num_projections=128):
    min_bs = min(z1.shape[0], z2.shape[0])
    
    idx1 = torch.randperm(z1.shape[0])[:min_bs]
    idx2 = torch.randperm(z2.shape[0])[:min_bs]
    z1_sub, z2_sub = z1[idx1], z2[idx2]

    dim = z1.shape[1]
    projections = torch.randn(dim, num_projections, device=z1.device)
    projections = nn.functional.normalize(projections, p=2, dim=0)

    z1_proj = torch.matmul(z1_sub, projections)
    z2_proj = torch.matmul(z2_sub, projections)

    z1_sorted, _ = torch.sort(z1_proj, dim=0)
    z2_sorted, _ = torch.sort(z2_proj, dim=0)

    return torch.mean((z1_sorted - z2_sorted) ** 2).item()
...
class TabularResNetMortalityModel(nn.Module):
    def __init__(self, input_dim, d_token=16, hidden_dim=256, num_blocks=2):
        ...
        self.blocks = nn.Sequential(*[PreActResNetBlockLN(hidden_dim) for _ in range(num_blocks)])
        self.final_ln = nn.LayerNorm(hidden_dim)
        self.final_relu = nn.ReLU()

        self.base = nn.Sequential(
            self.initial, self.blocks, self.final_ln, self.final_relu
        )
    ...
    def forward(self, x):
        return self.head(self.base(self._pooled_tokens(x)))

    def forward_full(self, x):
        latents = self.base(self._pooled_tokens(x))
        logits = self.head(latents)
        projections = self.projector(latents)
        return logits, latents, projections
...
def train_local_sgd(model, X, y, epochs=LOCAL_EPOCHS, lr=BASE_LR):
    model.train()
    ...
    for _ in range(epochs):
        for bx, by in loader:
            ...
            logits, _, projections = model.forward_full(bx)
...
algorithms = ['FedSWD_Weighted_SupCon']
...
for algo in algorithms:
    ...
    for target_hospital in target_hospitals:
        ...
        for h in source_hospital_ids:
            ...
            training_clients[h] = (X_tr, y_tr)
            source_val_data.append(X_val)

        X_global_ref = np.concatenate(source_val_data, axis=0)
        ...
        X_tgt_val = source_scaler.transform(X_val_raw)
        X_tgt_test = source_scaler.transform(X_test_raw)
        ...
        for r in range(FED_ROUNDS):
            ...
            # extract global reference representations
            Z_ref = []
            ref_loader = DataLoader(TensorDataset(torch.tensor(X_global_ref, dtype=torch.float32)), batch_size=1024)
            with torch.no_grad():
                for (bx,) in ref_loader:
                    _, latents, _ = global_model.forward_full(bx.to(device))
                    Z_ref.append(latents)
            Z_ref = torch.cat(Z_ref, dim=0)

            # extract local client representations and compute swd distance
            for hospital, (X, y) in training_clients.items():
                client_sizes.append(len(X))
                
                Z_local = []
                loc_loader = DataLoader(TensorDataset(torch.tensor(X, dtype=torch.float32)), batch_size=1024)
                with torch.no_grad():
                    for (bx,) in loc_loader:
                        _, latents, _ = global_model.forward_full(bx.to(device))
                        Z_local.append(latents)
                Z_local = torch.cat(Z_local, dim=0)

                swd_val = compute_swd_distance(Z_local, Z_ref)
                swd_scores.append(swd_val)
                
            # compute swd similarity weights
            swd_scores = np.array(swd_scores)
            min_swd, max_swd = np.min(swd_scores), np.max(swd_scores)
            swd_norm = (swd_scores - min_swd) / (max_swd - min_swd + 1e-8)
            S_k = np.exp(-BETA_PARAM * swd_norm)
            
            n_k = np.array(client_sizes)
            unnormalized_weights = n_k * S_k
            agg_weights = unnormalized_weights / np.sum(unnormalized_weights)

            # local training
            for hospital, (X, y) in training_clients.items():
                local_model = TabularResNetMortalityModel(input_dim).to(device)
                local_model.load_state_dict(global_state)
                trained_state = train_local_sgd(local_model, X, y, epochs=LOCAL_EPOCHS, lr=round_lr)
                client_weights.append(trained_state)

            # swd-weighted aggregation
            for key in global_state.keys():
                tensors = [w[key] for w in client_weights]
                if tensors[0].dtype.is_floating_point:
                    global_state[key] = sum(tensors[i].float() * agg_weights[i] for i in range(len(tensors)))
                else:
            ...
            val_metrics = evaluate_comprehensive(eval_model, X_tgt_val, y_val)
        ...
        test_metrics = evaluate_point_and_bootstrap_ci(
            eval_model, X_tgt_test, y_test, fixed_threshold=locked_test_threshold, n_boot=100, seed=SEED
        )
...
output_csv = "resnet_supcon_focal_swd_results.csv"
