...
class FTTransformerMortalityModel(nn.Module):
    def __init__(self, input_dim, d_token=64, n_heads=2, num_layers=1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x_seq = self.emb_dropout(x_seq)

        out_seq = self.transformer_encoder(x_seq)
        cls_out = out_seq[:, 0, :]
        return self.head(cls_out)

# tokenized pre-activation resnet architecture
...
class TokenizedResNetMortalityModel(nn.Module):
...
# local training routine using AdamW + Cosine Decay
...
# main training loop for comparison
model_configs = {
    "FT-Transformer": FTTransformerMortalityModel,
    "Tokenized-ResNet": TokenizedResNetMortalityModel,
}
all_benchmark_records = []

for model_name, ModelClass in model_configs.items():
    print(f"\n--- Running model architecture: {model_name} ---")

    for target_hospital in target_hospitals:
        print(f"\ntarget hospital fold: {target_hospital} ({model_name})")
        ...
        global_model = ModelClass(input_dim).to(device)
        ...
        for r in range(FED_ROUNDS):
            ...
            for hospital, (X, y) in training_clients.items():
                local_model = ModelClass(input_dim).to(device)
            ...
            eval_model = ModelClass(input_dim).to(device)
...
output_csv = "transformer_resnet_comparison_results.csv"
...
for model_name in model_configs.keys():
    sub = test_rows[test_rows["Model"] == model_name]
    print(f"\n{model_name}:")
