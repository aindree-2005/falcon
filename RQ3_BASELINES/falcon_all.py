...
# metadata columns (ventilation and sepsis included in feature_cols for full ablation)
...
# base tabular features from ready csv
base_feature_cols = df.columns.difference(meta_cols)

# non-lab feature indices (used strictly for SWD & prototype calculation)
non_lab_cols = [c for c in base_feature_cols if c not in lab_cols]

# load pre-extracted federated LOHO diagnosis and treatment embeddings per fold
clients_raw_folds = {}
for target_hospital in target_hospitals:
    diag_emb_path = f"/kaggle/working/diagnosis_embeddings_LOHO_target_{target_hospital}.npy"
    if not os.path.exists(diag_emb_path):
        diag_emb_path = f"/kaggle/input/datasets/aindreechatterjee/ablation-dafl/diagnosis_embeddings_LOHO_target_{target_hospital}.npy"
        
    treat_emb_path = f"/kaggle/working/treatment_embeddings_LOHO_target_{target_hospital}.npy"
    if not os.path.exists(treat_emb_path):
        treat_emb_path = f"/kaggle/input/datasets/aindreechatterjee/ablation-dafl/treatment_embeddings_LOHO_target_{target_hospital}.npy"

    if os.path.exists(diag_emb_path) and os.path.exists(treat_emb_path):
        diag_emb = np.load(diag_emb_path)
        treat_emb = np.load(treat_emb_path)
        X_fold_full = np.concatenate([df[base_feature_cols].values, diag_emb, treat_emb], axis=1)
    else:
        X_fold_full = df[base_feature_cols].values

    fold_clients = {}
    total_feat_dim = X_fold_full.shape[1]
    
    for h in selected_hospitals:
        h_mask = (df["hospitalid"] == h).values
        fold_clients[h] = (X_fold_full[h_mask], df["death"].values[h_mask])
    clients_raw_folds[target_hospital] = (fold_clients, total_feat_dim)

# compute non-lab indices dynamically based on the merged feature dimensions
non_lab_indices = [list(base_feature_cols).index(c) for c in non_lab_cols]

print(f"created client partitions with full embeddings")
print(f"base features = {len(base_feature_cols)}, non-lab features for SWD = {len(non_lab_cols)}")
...
algorithms = ['FALCON_Full_Embeddings_Ablation']
...
for algo in algorithms:
    ...
    for target_hospital in target_hospitals:
        ...
        fold_clients, input_dim = clients_raw_folds[target_hospital]
        source_hospital_ids = [h for h in fold_clients.keys() if h != target_hospital]
        
        # scale features using source hospital data only
        X_source_concat = np.concatenate([fold_clients[h][0] for h in source_hospital_ids], axis=0)
        ...
        X_tgt_raw, y_tgt = fold_clients[target_hospital]
...
output_csv = "falcon_full_embeddings_results.csv"
df_results = pd.DataFrame(all_benchmark_records)
df_results.to_csv(output_csv, index=False)

test_rows = df_results[df_results["Type"] == "Test"]
auroc_mean, auroc_std = test_rows["AUROC"].mean(), test_rows["AUROC"].std()
auprc_mean, auprc_std = test_rows["AUPRC"].mean(), test_rows["AUPRC"].std()
f1_mean, f1_std = test_rows["F1"].mean(), test_rows["F1"].std()
prec_mean, prec_std = test_rows["Precision"].mean(), test_rows["Precision"].std()
rec_mean, rec_std = test_rows["Recall"].mean(), test_rows["Recall"].std()
...
print(f"\nLaTeX Table Row:")
print(f"\\shortstack{{Demographics + Labs + Medicines\\\\\n+ Sepsis + Ventilation + Treatment\\\\\n+ Diagnosis Time-Series Embeddings}} & \\shortstack{{{auroc_mean:.4f}\\\\{auroc_std:.4f}}} & \\shortstack{{{auprc_mean:.4f}\\\\{auprc_std:.4f}}} & \\shortstack{{{f1_mean:.4f}\\\\{f1_std:.4f}}} & \\shortstack{{{prec_mean:.4f}\\\\{prec_std:.4f}}} & \\shortstack{{{rec_mean:.4f}\\\\{rec_std:.4f}}} \\\\")
print("\\hline")
