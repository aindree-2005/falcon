...
def train_local_falcon(model, X, y, epochs=LOCAL_EPOCHS, lr=BASE_LR, global_weights=None, global_proto=None):
    model.train()
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=WEIGHT_DECAY)
    y_t = torch.tensor(y, dtype=torch.float32)
    pos = y_t.sum().item(); neg = len(y_t) - pos
    criterion = FocalLoss(alpha=neg/(pos+neg+1e-8), gamma=2.0).to(device)
    ...
    for _ in range(epochs):
        for bx, by in loader:
            ...
            la = 0.05 * torch.nn.functional.mse_loss(lat.mean(dim=0), global_proto) if global_proto is not None else 0.0
        ...
        for r in range(FED_ROUNDS):
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(X_global_ref_t), 1024):
                    _, l = global_model(X_global_ref_t[i:i+1024])
            ...
            for hospital, client_data in training_clients.items():
                ...
                with torch.no_grad():
                    ...
                    for i in range(0, len(X_local_t), 1024):
                        _, l = local_model(X_local_t[i:i+1024].to(device))
                        l_latents.append(l)
            ...
            meta_criterion = FocalLoss(alpha=neg_c / (pos_c + neg_c + 1e-8), gamma=2.0).to(device)
...
output_csv = "falcon_ultimate_results.csv"
